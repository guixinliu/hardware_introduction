...
md"""
# 科学家为编写快速代码所必须了解的硬件知识

**本手册的英文版位于 https://github.com/jakobnissen/hardware_introduction**

如今许多科学领域都用到了编程，这些领域内的科学家们通常不得不为自己的项目编写定制的代码。然而，对于大多数科学家来说，计算机科学不是他们的专业领域；他们不可避免地需要学习编程。我认为自己就是其中之一。虽然我们可能对编程的**软件**方面比较熟悉，但是我们对计算机**硬件**如何影响代码性能甚至很少有基本的认识。

本教程的目标是向业余编程选手提供现代硬件特性的**简明**概述，其中只包含了编写快速代码所必须了解的特性。这将是对过去几年所学知识的升华。本教程将使用 Julia， 因为它能够使用高级交互式语言轻松地演示这些相对低层级的考虑。
...
#### 这不是关于代码基准测试的教程
**实际**编写快速代码时，测试代码查找瓶颈是非常有必要的，即找出机器消耗大多数时间的地方。通常必须需要对不同的函数和方法进行基准测试，才能找到最快实践。Julia（以及其他语言）为此提供了工具，但此处不做介绍。
"""
